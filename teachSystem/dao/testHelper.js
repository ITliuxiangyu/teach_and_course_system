...
class testHelper {
  ...
  static choseQuestion(sum, type, num, levelNum, questions) {
    ...
    for (let i = 0; i < num; i++) {
      if (keys.length == 0) {
        throw ErrorHelper.Error403("出题失败，备选题目数量不足,请联系教师扩充1111111");
      }
      ...
      if (tempQuestion == undefined || tempQuestion.length == 0) {
        throw ErrorHelper.Error403("出题失败，难度系数为"+keys[random1]+"的备选题目数量不足,请联系教师扩充22222");
      }
  ...
  static async getQuestionGroup(questionData, persent, isExam) {
    let questionNum = parseInt(questionData.question.single.num) + parseInt(questionData.question.mult.num) + parseInt(questionData.question.gap.num) + parseInt(questionData.question.program.num) + parseInt(questionData.question.jianda.num);
    //查询选中章节下的题目
    let knowledgePont;
    ...
    testHelper.choseQuestion(sum, "program", parseInt(questionData.question.program.num), levelNum, data.program);

    testHelper.choseQuestion(sum, "jianda", parseInt(questionData.question.jianda.num), levelNum, data.jianda);
