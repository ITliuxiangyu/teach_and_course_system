<template>
  <div>
    <el-collapse accordion v-if="isFailed==false" v-loading="isLoading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)">
      ...
      <el-collapse-item name="1" v-if="data.single && data.single.length>0">
        <template slot="title">
      ...
      <el-collapse-item name="2" v-if="data.mult && data.mult.length>0">
        <template slot="title">
      ...
      <el-collapse-item name="3" v-if="data.gap && data.gap.length>0">
        <template slot="title">
      ...
      <el-collapse-item v-if="data.program && data.program.length>0" name="4">
        <template slot="title">
      ...
      <el-collapse-item v-if="data.jianda && data.jianda.length>0" name="5">
        <template slot="title">
          <h3>&nbsp;<i class="el-icon-caret-right"></i>简答题</h3>
        </template>
        <do-question v-for="(item,index) in data.jianda" :key="index" :index="index+1" :data="item" :value="answers[item.ql_id]" :sum="sum[item.ql_id]" :action="action"></do-question>
      </el-collapse-item>
