        ...
        <div class="show-selected">
          ...
          <el-collapse v-model="activeName" accordion>
            ...
            <el-collapse-item :title="'编程题，共：'+form.onSystemQuestion[3].length" name="4">
              <question-list v-on:change_score="change_score"  :data="form.onSystemQuestion[3]"></question-list>
            </el-collapse-item>
            <el-collapse-item :title="'简答题，共：'+form.onSystemQuestion[4].length" name="5">
              <question-list :data="form.onSystemQuestion[4]"></question-list>
            </el-collapse-item>
          </el-collapse>
        </div>
      </div>
      ...
      <div v-show="currentStep==3" style="width:600px">
        <el-form label-width="100px">
          ...
          <el-form-item label="题目">
            ...
            <div>简答题：{{form.onSystemQuestion[4].length}}道， 共{{num5()}}分</div>
            <div>总计：{{num1()+num2()+num3()+num4() + num5()}}分</div>
          </el-form-item>
        </el-form>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        qg_type: "",
        demand: "",
        onSystemQuestion: [[], [], [], [] , []], //单选，多选，填空,编程 , 简答
        difficulty: 0
      }
    };
  },
  ...
  methods: {
    ...
    num5() {
      let num = 0;
      this.form.onSystemQuestion[4].forEach(item => (num += parseInt(item.score)));
      return num;
    },
    change_score(aaa){
      // console.log("监听到时间....")
      // console.log("111111")
      // console.log(aaa)
      // var tmp_onSystemQuestion = this.form.onSystemQuestion
      // tmp_onSystemQuestion[3][1].score = 111

      // console.log(this.form.onSystemQuestion)
      // this.$set(this.form , "onSystemQuestion" , tmp_onSystemQuestion)
    } , 
    nextStep(){
    ...
    addOnSystemQuestion(data) {
      ...
      data.forEach(item => {
        ...
        else if (item.q_type == 5) item.score = 30;
    ...
    submit() {
      this.form.score = this.num1() + this.num2() + this.num3() + this.num4() + this.num5();
      $http
