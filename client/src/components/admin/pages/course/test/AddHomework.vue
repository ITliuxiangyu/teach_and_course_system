  ...
  <div>
    ...
    <el-form label-width="100px" v-show="currentStep==1">
      <el-form-item label="作业名称">
        <el-input v-model="form.test_name" style="width:400px"></el-input>
      </el-form-item>
      <el-form-item label="作业描述">
        <el-input type="textarea" style="width:250px" :autosize="{minRows:2,maxRows:5}" placeholder="请输入作业要求" v-model="form.demand"></el-input>
      </el-form-item>
      <el-form-item label="开始时间">
        <el-date-picker v-model="form.start_time" format="yyyy-MM-dd HH:mm" type="datetime" placeholder="选择开始时间"></el-date-picker>
      </el-form-item>
      <el-form-item label="结束时间">
        <el-date-picker v-model="form.end_time" format="yyyy-MM-dd HH:mm" type="datetime" placeholder="选择结束时间"></el-date-picker>
      </el-form-item>

    </el-form>
    <!-- 第二步 -->
    <chose-homework-question v-show="currentStep==2" v-on:questionChange="changeHomeworkQuestion" ref="choseQuestion">
    </chose-homework-question>
    <!-- 第三部 -->
    <chose-class v-show="currentStep==3" v-on:class-change="classChange"></chose-class>
    <!-- 第四部 -->
    <div v-show="currentStep==4">
      <el-form label-width="100px">
        <el-form-item label="作业名">{{form.test_name}}</el-form-item>
        <el-form-item label="考试要求">
          <pre style="line-height:20px">{{form.demand}}</pre>
        </el-form-item>
        <el-form-item label="开始时间">{{startTime}}</el-form-item>
        <el-form-item label="结束时间">{{endTime}}</el-form-item>
...
<script>
import ChoseHomeworkQuestion from "./common/ChoseHomeworkQuestion";
import ChoseClass from "./common/ChoseClass";
export default {
  name: "AddHomework",
  components: {
    ChoseHomeworkQuestion,
    ChoseClass
  },
  data() {
    return {
  ...
  computed: {
    ...
    endTime() {
      return this.$moment(this.form.end_time).format("YYYY-MM-DD hh:mm");
    }
  },
  created() {
